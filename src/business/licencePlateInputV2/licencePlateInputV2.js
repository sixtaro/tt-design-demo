...
export default function LicencePlateInputV2(props) {
    const { disabled = false, canInputKeys = [], hideBrief = [], plateColorValueFromOutside, type = 'search', onColorOptionsChange, popoverToTop, copy = true } = props;
    ...
    const plateNumberBoxClass = useMemo(() => {
        let classList = ['licence-plate-box'];
        // 如果此时不为虚拟车牌输入，则需要展示中间的圆点（或者其他标识）
        if (plateLength === 8) {
            classList.push('with-dot');
        }
        classList.push(plateColor);
    ...
    useEffect(() => {
        if (focus && cursor >= 0 && inputRefs.current[cursor]) {
    ...
    return (
        <div className="licence-plate-v2" ref={licencePlateRef}>
            <div className={plateNumberBoxClass} tabIndex="0" onClick={handleClickAround}>
                <Popover
                    overlayClassName={`${cursor === 0 ? 'licence-plate-box-popover' : ''} ${popoverToTop ? 'popoverToTop' : ''}`}
                    content={
                        cursor === 0 ? (
                            <div className="licence-plate-box__dropdown">
                                {provinceBriefList
                ...
                >
                    {plateCharList &&
                        plateCharList.map((item, index) => {
                            let className =
                                'input-box' +
                                (index === cursor && focus ? ' focus' : '') +
                                (index === plateLength - 1 && !plateCharList[index] ? ' xinnengyuan' : '');
                            return (
                                <div
                ...
                {!disabled && <CloseOutlined className="clear" onClick={clear} />}
                {copy && <div className="copy" style={{ right: !disabled ? '-38px' : '-20px' }}>
                    <CopyIcon text={value || ''} />
                </div>}
            </div>
        </div>
